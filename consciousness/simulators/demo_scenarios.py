...
class DemoScenarios:
    ...
    async def smart_morning_routine(simulator: "HouseSimulator"):
        ...
        bedroom_lights = [d for d in simulator.get_device_by_location("bedroom") 
                         if d.device.device_class == "light"]
        kitchen_lights = [d for d in simulator.get_device_by_location("kitchen") 
                         if d.device.device_class == "light"]
        ...
        for climate in climate_devices:
            current_temp = climate.state["temperature"]
        ...
        motion_sensors = [d for d in simulator.get_device_by_class("sensor")
        ...
        for energy in energy_devices:
            print(f"Energy Status - Solar: {energy.state['solar_production']}kW, "
    ...
    async def security_alert_scenario(simulator: "HouseSimulator"):
        ...
        motion_sensors = [d for d in simulator.get_device_by_class("sensor")
                         if d.device.capabilities.get("sensor_type") == "motion"]
        door_sensors = [d for d in simulator.get_device_by_class("sensor")
        ...
        for security in security_devices:
            alert = {
                "type": "intrusion",
                "location": motion_sensors[0].device.location if motion_sensors else "unknown",
        ...
        exterior_lights = [l for l in all_lights if "exterior" in l.device.location.lower()]
    ...
    async def energy_optimization_scenario(simulator: "HouseSimulator"):
        ...
        for climate in climate_devices:
            await climate.set_state({
                "target_temperature": 70.0,  # Pre-cool
                "fan_speed": "high"
            })
        ...
        print(f"✅ Energy Optimization completed - Saved ${random.uniform(2.5, 5.0):.2f} today")
    ...
    async def party_mode_scenario(simulator: "HouseSimulator"):
        ...
        for climate in climate_devices:
            await climate.set_state({
                "target_temperature": 68.0,  # Cooler for crowds
                "fan_speed": "high"
            })
        ...
        kitchen_lights = [l for l in all_lights if "kitchen" in l.device.location.lower()]
    ...
    async def vacation_mode_scenario(simulator: "HouseSimulator"):
        ...
        for day in range(3):  # Simulate 3 days
            ...
            living_lights = [l for l in all_lights if "living" in l.device.location.lower()]
            bedroom_lights = [l for l in all_lights if "bedroom" in l.device.location.lower()]
        ...
        if energy_devices:
            ...
            await energy.set_state({
                ...
                "total_energy": energy.state["total_energy"] + 12  # 3 days minimal usage
            })
